...
if __name__ == "__main__":
    ...
    df2['poss_correspondencia'] = df2['id'].apply(lambda x: True if x in truth["antigoId"].unique() else False)

    # Pré-processar correspondências para evitar overhead durante blocking
    print("Pré-processando correspondências...")
    st_prep = time.time()
    
    # Criar índice reverso df2: id -> index
    ...
    for index1 in range(len(df1)):
        ncid = df1.iloc[index1]['ncid']
        correspondencias = []
        
        if ncid in truthD:
            ids_scholar = truthD[ncid]
            for id_scholar in ids_scholar:
