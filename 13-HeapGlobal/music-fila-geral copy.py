...
def matching(valores_para_matching):
    global tp, fp, pairsNo, L1, q, matched_pairs
        ...
        for correspondencias in valores_para_matching:
            for index2 in correspondencias: 
                ...
                for id in matchingPairs.keys():
                    ...
                    # Criar chave única para o par (evita duplicatas)
                    pair_key = (idDBLP, ncid)
                    if pair_key in matched_pairs:
                        continue  # Par já contado
...
if __name__ == "__main__":
    # Carregar datasets PRÉ-PROCESSADOS (sem overhead)
    # Execute 'python prepare_data.py' UMA VEZ antes de rodar este script
    df1 = pd.read_csv("musicbrainz-200-A01_prepared.csv", sep=",", encoding="utf-8", keep_default_na=False)
    df2 = pd.read_csv("music_brainz-simple-mutated_prepared.csv", sep=",", encoding="utf-8", keep_default_na=False)
    ...
    # df2 já possui coluna 'poss_correspondencia' do arquivo preparado
    # df1 já possui coluna 'correspondencias_indices' do arquivo preparado
    ...
    fp = 0
    pairsNo = 0
    matched_pairs = set()  # Rastrear pares únicos (evita duplicatas)
    nbS = 1
    naS = 1
    offsetA = 50
    offsetB = 50
    ...
    while True:
        ...
        for index1 in range(naS, naS + offsetA):
            ...
            # Ler correspondências diretamente do CSV pré-processado (ZERO overhead)
            correspondencias = json.loads(df1.iloc[index1]['correspondencias_indices'])
